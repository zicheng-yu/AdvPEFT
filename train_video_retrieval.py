...
from data import create_dataset, create_sampler, create_loader, didemo_collate
...
def train(model, data_loader, optimizer, epoch, device, config):
    ...
    metric_logger.add_meter('loss_align', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    ...
    for i, (image, caption, idx) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        adv_video = utils.pgd_attack(model, image, caption, alpha, idx, config)
        loss_ita, loss_itm = model(adv_video, caption, alpha=alpha, idx=idx)
        loss_align = model_without_ddp.alignment_loss()
        loss = loss_ita + loss_itm + config['align_loss_weight'] * loss_align
...
def evaluation(model, data_loader, device, config):
    ...
    for video, video_id in data_loader:
        batch_size, num_frames, channels, width, height = video.size()
        video = video.view(-1, channels, width, height).to(device, non_blocking=True)

        video_feat = model.visual_encoder(video)
        video_embed = model.vision_proj(video_feat[:, 0, :])
        video_embed = video_embed.view(batch_size, num_frames, -1).mean(dim=1)
        video_embed = F.normalize(video_embed, dim=-1)

        video_feat = video_feat.view(batch_size, -1, video_feat.shape[-1])
...
def main(args, config):
    ...
    train_dataset, test_dataset = create_dataset('retrieval_%s' % config['dataset'], config)
    ...
    collate_fn = didemo_collate if config['dataset'] == 'didemo' else None
    ...
    total_param = model.configure_trainable_parameters()
    ...
    for epoch in range(0, config['max_epoch']):
        if not args.evaluate:
            ...
            if utils.is_main_process():
                ...
                with open(os.path.join(args.output_dir, "log.txt"), "a+") as file_obj:
                    file_obj.write(json.dumps(log_stats) + "\n")
        ...
        if args.evaluate:
            ...
            if utils.is_main_process():
                ...
                with open(os.path.join(args.output_dir, "evaluate.txt"), "a+") as file_obj:
                    file_obj.write(json.dumps(log_stats) + "\n")
            break
