...
class Mlp(nn.Module):
    ...
    """
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., R = 64):
        super().__init__()
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., R = 64):
        ...
        self.R = R
        self.CP_attention = nn.Parameter(torch.zeros(self.R, 4))
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, CP_U=None, CP_V=None, CP_C=None):
        # x (bs, C, H, W)
        B = x.shape[0]
        x = self.patch_embed(x) # (bs, seqlen, hidden_dim)
        ...
        for i,blk in enumerate(self.blocks):
            x = blk(x, register_blk==i, CP_U, CP_V, CP_C)
